...
function Post() {
    ...
    const getPost = async () => {
        const res = await fetch(`http://localhost:3000/api/post/getPost?postId=${postId?.id}`)
      ...
      const getUser = async () =>{
        ...
        try{
          const res = await fetch(`http://localhost:3000/api/user/getUser?userId=${posts[0]?.owner}`)
          const data = await res.json()
      ...
      const getComment = async () =>{
        try{
          const res = await fetch(`http://localhost:3000/api/comment/getComment?postId=${posts[0]?._id}`)
      ...
      const handleSubmit = async(e) => {
        ...
        try{
            if(currentUser){
                const res = await fetch('http://localhost:3000/api/comment/createComment', {
                    method: 'POST',
      ...
      const likeComment = async (commentId) => {
        try{
            if(currentUser){
                const res = await fetch(`http://localhost:3000/api/comment/likeComment/${commentId}/${currentUser?._id}`, {
                    method: 'PUT',
                })
